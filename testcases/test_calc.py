...
class TestCalculator:
    ...
    @allure.story("加法功能")
    @allure.step("加法正向用例")
    @pytest.mark.parametrize('a,b,expect', datas=get_calc_data("add_success")[0], ids=get_calc_data("add_success")[1])
    def test_add_success(self, a, b, expect):
        assert expect == self.calc.add(a, b)

    @allure.step("加法反向用例")
    @pytest.mark.parametrize('a,b,expect', datas=get_calc_data("add_fail")[0], ids=get_calc_data("add_fail")[1])
    ...
    @allure.story("除法功能")
    @allure.step("除法正向用例")
    @pytest.mark.parametrize('a,b,expect', datas=get_calc_data("div_success")[0], ids=get_calc_data("div_success")[1])
    def test_div(self, a, b, expect):
        assert expect == self.calc.div(a, b)

    @allure.step("除法反向用例")
    @pytest.mark.parametrize('a,b,expect', datas=get_calc_data("div_fail")[0], ids=get_calc_data("div_fail")[1])
    def test_div(self,a,b,expect):
